...
# -*- coding: utf-8 -*-
r"""
Скрипт применения и синхронизации локализации для мода 'naturescompass'.
1. Считывает переводы из new_translate.md.
2. Сохраняет в translations/mods/naturescompass.json.
3. Записывает напрямую в D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\naturescompass\lang\ru_ru.json.
...
LOCAL_MOD_JSON = os.path.join(ROOT_DIR, "translations", "mods", "naturescompass.json")
...
def apply_and_sync():
    ...
    print(f"💾 Обновлён translations/mods/naturescompass.json (всего {len(local_data)} ключей)")
