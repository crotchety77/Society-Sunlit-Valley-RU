...
# -*- coding: utf-8 -*-
"""
Скрипт применения и синхронизации локализации для мода 'moreminecarts'.
1. Считывает переводы из new_translate.md.
2. Сохраняет в translations/mods/moreminecarts.json.
3. Записывает напрямую в D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\moreminecarts\lang\ru_ru.json.
...
LOCAL_MOD_JSON = os.path.join(ROOT_DIR, "translations", "mods", "moreminecarts.json")
GAME_RU_PATH = r"D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\moreminecarts\lang\ru_ru.json"
...
def apply_and_sync():
    ...
    print(f"💾 Обновлён translations/mods/moreminecarts.json (всего {len(local_data)} ключей)")
