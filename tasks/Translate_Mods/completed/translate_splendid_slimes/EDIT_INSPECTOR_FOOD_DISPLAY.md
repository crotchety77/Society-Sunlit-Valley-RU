# 🔬 Полный шаблон и код Анализатора слаймов (slimeInspectorEnhanced.js)

В этом файле представлен **весь скрипт Анализатора целиком**: все списки любимой еды, цены плортов, логика проверки билетов и отправка сообщений в чат.

---

## 🎨 1. Визуальный макет (как это теперь отображается в игре)

### 🟢 Одиночный слайм:
```text
==================================================
  [ Анализ слайма ]: Песчаный Слайм
--------------------------------------------------
  ▪ Рацион питания: Кости и костяная рыба (Подробнее в JEI)
  ▪ Любимое лакомство: 🔒 Требуется Билет слайма для раскрытия
  ▪ Производство плорта: [Песчаный плорт] — 64 ●
  ▪ Состояние: Сытость: Сыт | Настроение: Счастливый
==================================================
```
...
```text
==================================================
  [ Анализ слайма ]: Ларго (Плакса Слайм + Золото-рудный Слайм)
--------------------------------------------------
  ▪ Рацион Ларго: Древесные фрукты и Консервация и джемы (Подробнее в JEI)
  ▪ Любимое лакомство: [Банан] или 🔒 Требуется Билет слайма для раскрытия
  ▪ Производство плортов Ларго:
    ▪ [Плакса плорт] — 320 ●
    ▪ [Золото-рудный плорт] — 2048 ●
  ▪ Состояние: Сытость: Сыт | Настроение: Счастливый
==================================================
```
...
## 💻 2. Полный рабочий код Анализатора (активен в игре):
...
(() => {
  ...
  ItemEvents.entityInteracted((e) => {
    ...
    if (isLargo) {
      ...
      player.tell(
        Text.of("  §f▪ Рацион питания: §f").append(
          Text.translatable(`diet.splendid_slimes.${breed}`)
          .append(" §8(Подробнее в JEI)") 
        )
      );
      player.tell(
        Text.of("  §d▪ Любимое лакомство: §e").append(
          getFavoriteFoodDisplay(breed)
        )
      );
      player.tell(
...
## 🎨 3. Справочник цветов и стилей Minecraft:

| Код | Цвет / Стиль | Код | Цвет / Стиль |
|---|---|---|---|
| `§f` | Белый | `§a` | Зелёный |
| `§7` | Светло-серый | `§b` | Голубой (Аква) |
| `§8` | Тёмно-серый | `§c` | Красный |
| `§e` | Жёлтый | `§d` | Розовый |
| `§6` | Золотой / Оранжевый | `§9` | Синий |
| `§l` | **Жирный (Bold)** | `§o` | *Курсив (Italic)* |
| `§n` | <u>Подчёркнутый</u> | `§r` | Сброс цвета |
| `\n` | Перенос строки во всплывающем окне (`hover`) | | |
