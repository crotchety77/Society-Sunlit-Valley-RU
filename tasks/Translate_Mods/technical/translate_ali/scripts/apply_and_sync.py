...
# -*- coding: utf-8 -*-
r"""
Скрипт применения и синхронизации локализации для мода 'ali'.
1. Считывает переводы из new_translate.md.
2. Сохраняет в translations/mods/ali.json.
3. Записывает напрямую в D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\ali\lang\ru_ru.json.
...
LOCAL_MOD_JSON = os.path.join(ROOT_DIR, "translations", "mods", "ali.json")
GAME_RU_PATH = r"D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\ali\lang\ru_ru.json"
...
def apply_and_sync():
    ...
    print(f"💾 Обновлён translations/mods/ali.json (всего {len(local_data)} ключей)")
