...
# -*- coding: utf-8 -*-
r"""
Скрипт применения и синхронизации локализации для мода 'cloth-config2'.
1. Считывает переводы из new_translate.md.
2. Сохраняет в translations/mods/cloth-config2.json.
3. Записывает напрямую в D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\cloth-config2\lang\ru_ru.json.
...
LOCAL_MOD_JSON = os.path.join(ROOT_DIR, "translations", "mods", "cloth-config2.json")
GAME_RU_PATH = r"D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\cloth-config2\lang\ru_ru.json"
...
def apply_and_sync():
    ...
    print(f"💾 Обновлён translations/mods/cloth-config2.json (всего {len(local_data)} ключей)")
