import os
import sys
import json
import re

if sys.platform == 'win32':
    sys.stdout.reconfigure(encoding='utf-8')

GAME_DIR = r'D:\ModrinthApp\profiles\Society_ Sunlit Valley\config\ftbquests\quests\chapters'

# Let's inspect each quest in SNBT files directly
snbt_files = [f for f in os.listdir(GAME_DIR) if f.endswith('.snbt')]

q_snbt_map = {}
for fname in snbt_files:
    fpath = os.path.join(GAME_DIR, fname)
    with open(fpath, 'r', encoding='utf-8') as f:
        content = f.read()
        
    blocks = content.split('{\n\t\t\tid: "')
    for b in blocks[1:]:
        qid_m = re.match(r'^([0-9A-Fa-f]{16})\"', b)
        if not qid_m:
            continue
        qid = qid_m.group(1).upper()
        
        # Extract title if present
        title_m = re.search(r'title:\s*\"([^\"]*)\"', b)
        title = title_m.group(1) if title_m else ''
        
        # Extract description
        desc_lines = []
        desc_m = re.search(r'description:\s*\[([\s\S]*?)\]', b)
        if desc_m:
            raw = desc_m.group(1)
            desc_lines = re.findall(r'\"((?:\\\"|[^\"])*)\"', raw)
            
        q_snbt_map[qid] = {
            'file': fname,
            'title': title,
            'desc_lines': desc_lines
        }

# Let's check the 16 quests where EN=0 in en_us.json
ru_only_qids = [
    '1BAF470C2BE0A492', '362B0DE04E15818D', '3C02C152B8677DF1', '3C54AC272D438F76',
    '477FFAD808985736', '539C1B8E3358BBE1', '5B7E846EEFCA5603', '62D0735680A06D71',
    '6BCAB845986C9D3A', '0A455A4E0D7D074E', '118A9A2B8922DA00', '145DAA4D0C7740A3',
    '49EB48B999881A3', '7FEA26673F89CE0', 'B3AAC4CEC0098A3', 'F643DD3F3B61A7D'
]

print("=== Checking 16 RU-only keys in SNBT ===")
for qid in ru_only_qids:
    info = q_snbt_map.get(qid.upper(), {})
    if not info:
        # Pad with zeros if needed (short hex)
        for k, v in q_snbt_map.items():
            if k.endswith(qid.upper()) or qid.upper().endswith(k):
                info = v
                break
    print(f"\nQID: {qid} | File: {info.get('file')} | Title in SNBT: {info.get('title')}")
    print(f"Description lines in SNBT: {len(info.get('desc_lines', []))}")
    for l in info.get('desc_lines', []):
        print(f"  snbt line: {l}")
