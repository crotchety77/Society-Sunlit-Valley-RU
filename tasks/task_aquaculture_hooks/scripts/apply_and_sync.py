...
# -*- coding: utf-8 -*-
"""
Скрипт применения и синхронизации локализации крючков Aquaculture 2.
1. Считывает переводы из new_translate.md.
2. Сохраняет в translations/mods/aquaculture.json.
3. Записывает напрямую в D:\\ModrinthApp\\profiles\\Society_ Sunlit Valley\\kubejs\\assets\\aquaculture\\lang\\ru_ru.json.
...
LOCAL_MOD_JSON = os.path.join(ROOT_DIR, "translations", "mods", "aquaculture.json")
GAME_RU_PATH = r"D:\ModrinthApp\profiles\Society_ Sunlit Valley\kubejs\assets\aquaculture\lang\ru_ru.json"
...
def apply_and_sync():
    ...
    print(f"📦 Считано готовых строк перевода крючков: {len(new_trans)}")
    ...
    print(f"💾 Обновлён translations/mods/aquaculture.json (всего {len(local_data)} ключей)")
    ...
    if os.path.exists(sync_js):
        res = subprocess.run(["node", sync_js], cwd=ROOT_DIR, capture_output=True, text=True, encoding='utf-8', errors='replace')
    ...
    if os.path.exists(GAME_RU_PATH):
        ...
        if new_trans:
            print("Примеры применённых строк крючков:")
            for k in list(new_trans.keys())[:8]:
