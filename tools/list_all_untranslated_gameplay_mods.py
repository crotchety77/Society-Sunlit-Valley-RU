...
if __name__ == '__main__':
    mods = get_all_untranslated_mods()
    gameplay = [v for v in mods.values() if v['is_content']]
    print(f"Всего игровых модов с непереведёнными строками: {len(gameplay)}")
    print("\n--- Игровые моды (сортировка по количеству непереведённых строк) ---")
    for i, v in enumerate(sorted(gameplay, key=lambda x: x['untrans'], reverse=True), 1):
        task_status = "✅ Есть папка" if v['has_task'] else "❌ НЕТ ПАПКИ"
        print(f"{i:2d}. [{v['ns']}]: {v['untrans']} из {v['total']} не переведено ({v['pct']}%) [{task_status}] ({v['jar']})")
